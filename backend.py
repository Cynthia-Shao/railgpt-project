...
def _build_strategy_prompt(user_query: str, chunks) -> str:
    ...
    return f"""你是高速铁路调度策略分析师。根据用户场景 and 规则上下文，输出调度策略选项。
...
def _async_load_knowledge():
    try:
        print("Backend starting background knowledge base loading...")
        _ensure_knowledge()
        print("Backend background knowledge base loaded successfully!")
...
def _general_reply(user_query: str) -> dict:
    ...
    if any(word in text for word in ("不对", "错了", "这不对")):
        answer = "你说得对，我刚才的判断需要重新校验。如果涉及运行图调整，我会优先按地点、时间窗和列车实际通过时刻筛选受影响列车，不会再默认选第一趟车。"
...
def _build_concise_dispatch_answer(event, optimizer_results: list[dict], optimized_plan_name: str, diff_rows: list[dict], strategy: dict | None = None, detail: str = "") -> str:
...
def process_query(user_query: str, use_llm_strategy: bool = True) -> dict:
    ...
    if optimizer_results:
        result = SimpleNamespace(answer="", retrieved_chunks=retrieved_for_strategy or rag_service.retrieval_service.search(user_query, top_k=3))
    else:
        try:
            result = rag_service.answer(user_query, extra_timetable_context=optimizer_context)
        except RuntimeError as exc:
            fallback_answer = f"本地大模型服务暂不可用，系统先返回算法生成的调度结果。\n\n{optimizer_context or '当前问题未识别出可直接优化的运行图扰动。'}\n\nLLM错误: {exc}"
            result = SimpleNamespace(answer=fallback_answer, retrieved_chunks=rag_service.retrieval_service.search(user_query, top_k=3))
    ...
    for chunk in result.retrieved_chunks:
        ...
        if key not in seen:
            seen.add(key)
            priority_label = "场景流程" if chunk.priority == "scenario" else "强规则" if chunk.must_follow else "普通规则"
            references.append({"title": chunk.title, "source": chunk.source_path, "priority": priority_label})
    ...
    if intent:
        try:
            ...
            if intent["train_id"] and intent["delay_minutes"] > 0:
                plans = search_delay_parameter(rag_service.timetable, intent["train_id"], intent["delay_minutes"])
                ...
                plans = generate_speed_restriction_plans(rag_service.timetable, match_stn, intent["speed_limit"])
            if plans:
    ...
    if optimizer_results:
        ...
        try:
            if scenario_result:
                ...
                detail = explain_best_plan(optimizer_results[0], second, best_plan, original, event_train_id=event.train_id if event else "")
        except Exception:
            pass
        full_answer = _build_concise_dispatch_answer(event, optimizer_results, optimized_plan_name, _latest_diff_rows, strategy, detail=detail)
    else:
    ...
    return {
        "answer": full_answer,
        "plan": {"title": parsed["plan_title"] or "调度方案", "steps": parsed["steps"], "note": parsed["note"]},
...
# =========================================================================
# 【停站横线核心对齐层】：保留 Unnamed 奇偶到达出发列，融合成标准运行图双点对
# =========================================================================
...
def api_timetable():
    ...
    if version == "adjusted":
        ...
        df = _latest_adjusted_timetable
    else:
        df = pd.read_excel(xlsx_path, index_col=0)
        
    # 1. 建立标准的干净车站序列（只拿真正的站名）
    station_list = []
    for c in df.columns:
        c_str = str(c).strip()
        if c_str and not c_str.startswith("Unnamed"):
            station_list.append(c_str)
            
    trains = [str(t) for t in df.index]
    
    # 2. 构造前端统一的渲染数据流
    # 前端 ECharts 的 Y 轴依然是干净的 station_list。
    # 每一个车次的数据点，我们同时放入“到达时间”和“出发时间”，并让它们对应同一个车站的索引。
    times = []
    times_minutes = []
    ...
    for _, row in df.iterrows():
        ...
        for col_idx, cell in enumerate(row):
            ...
            if pd.isna(cell) or cell == "" or cell == "0" or cell == 0:
                continue
                
            try:
                val = int(round(float(cell)))
                # 存入数据点：[时间分钟数, 车站索引]
                row_min.append([val, current_station_idx])
                row_str.append(f"{station_list[current_station_idx]}: {val // 60:02d}:{val % 60:02d}")
            except:
                pass
                
        # 按照时间从早到晚给每趟车的点排序，防止连线错乱
        row_min.sort(key=lambda x: x[0])
        times_minutes.append(row_min)
        times.append(row_str)

    return jsonify({
        "stations": station_list,
        "trains": trains,
        "times": times,
        "times_minutes": times_minutes,   # 这里的结构变成了三维数组：[车次][打点][分钟数, 车站Y索引]
        "version": version,
...
if __name__ == "__main__":
    t = threading.Thread(target=_async_load_knowledge, daemon=True)
    t.start()
    
    print("RailGPT HTTP Server with Safe Dwell Horizontal Line Cleaner online on port 8000!")
